...
void Level::startLevel() {
  ...
  scene =
      new QGraphicsScene(0, 0, levelData->sceneSizeX, levelData->screenHeight);
  ...
  QObject::connect(player, SIGNAL(countChanged()), this,
                   SLOT(setInformationBar()));
  QObject::connect(player, SIGNAL(activeTimer()), this,
...
void Level::setView() {
  ...
  view->setBackgroundBrush(
      QPixmap(path).scaledToHeight(levelData->screenHeight));
  ...
  view->setWindowTitle(
      QString::fromStdString("Level " + std::to_string(levelId)));
  view->setFocus();
}


void Level::setInformationBar() { informationBar->updateInformation(); }

void Level::updateTimerLabel() {
  timerLabel->setText(
      "time: " + QString::number(levelTimer->remainingTime() / 1000) + " s");
...
void Level::outOfTime() { this->finishLevel(MessageText::OutOfTime); }
...
void Level::parseLevelMap() {
  ...
  for (int y = 0; y < sizeY; y++) {
    ...
    for (int x = 0; x < sizeX - 1; x++) {
      addObject(sceneObjects[x].toLatin1(), x * levelData->sceneSizeX / sizeX,
                y / sizeY);
    }
  }

...
Message::Message() {
  ...
  this->msgBox->setStyleSheet("font-size: 25px; font: bold; border-image: "
                              "url(:/images/messageBox.jpg) stretch;");

  messageTexts = {
      "Oops, you lost a life!", "Level over!",
...
QString Message::getMessage(MessageText messageValue) {
  if (messageValue < MessageText::LostLife ||
      messageValue > MessageText::OutOfTime)
    throw "Unknown message";

  return messageTexts[int(messageValue)];
}

Message::~Message() { delete this->msgBox; }

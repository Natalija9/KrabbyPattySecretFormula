...
TEST_CASE("getMessage", "[function]") {
  SECTION("Za prosledjenu vrednost koje se nalazi u MessageText funkcija vraca "
          "validan string") {
    // Arrange
    ...
    // Assert
    REQUIRE(QString::compare(ocekivani_izlaz, dobijen_izlaz,
                             Qt::CaseSensitive) == 0);
  }

  SECTION("Za prosledjeni int koji se nalazi u intervalu [0, 4] funkcija vraca "
          "validan string") {
    // Arrange
    Message* message = new Message();
    ...
    // Assert
    CHECK(QString::compare(ocekivani_izlaz1, dobijen_izlaz1,
                           Qt::CaseSensitive) == 0);
    REQUIRE(QString::compare(ocekivani_izlaz2, dobijen_izlaz2,
                             Qt::CaseSensitive) == 0);
  }

...
bool EqualLists(QList<std::pair<QString, int>>& a,
                QList<std::pair<QString, int>>& b) {
...
TEST_CASE("insertPlayerIntoList", "[function]") {
  ...
  SECTION("Ubacivanje igraca u listu na poziciju unutar postojece rang liste "
          "tako da lista ostane sortirana nerastuce") {
    // Arrange
    RangList* rang_list = new RangList();
    rang_list->readFromFileAndInsertIntoList();
  ...
  SECTION("Ubacivanje igraca sa ostvarenim brojem poena manjim od najmanjeg "
          "broja poena u postojecoj listi tako da lista ostane nepromenjena") {
    // Arrange
    RangList* rang_list = new RangList();
    rang_list->readFromFileAndInsertIntoList();

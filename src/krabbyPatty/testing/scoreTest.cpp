...
TEST_CASE("increase()", "[function]") {
  SECTION("Nakon kreiranja instance klase Score, trenutni skor treba da bude "
          "inicijalizovan na 0") {
    // Arrange
...
TEST_CASE("takeLife()", "[function]") {
  SECTION("Umanjivanje preostalog broja zivota za 1 i trenutni skor postavlja "
          "na 0") {
    // Arrange
...
TEST_CASE("countLevelScore()", "[function]") {
  SECTION("Izbacivanje izuzetka nakon odigranog nivoa ukoliko je preostalo "
          "vreme 0") {
    // Arrange
    Settings* settings = new Settings();
    LevelData* level_data = new LevelData(settings);
  ...
  SECTION(
      "Izbacivanje izuzetka nakon odigranog nivoa ukoliko je trenutni skor 0") {
    // Arrange
    Settings* settings1 = new Settings();
    LevelData* level_data1 = new LevelData(settings1);
    Score* score1 = new Score(level_data1);
...
TEST_CASE("getTotalScore", "[function]") {
  SECTION("Racunanje konacnog skora ukoliko nije sakupljen nijedan sastojak ni "
          "u jednom nivou") {
    // Arrange
    Settings* settings = new Settings();
    LevelData* level_data = new LevelData(settings);
...
TEST_CASE("saveCurrentScore()", "[function]") {
  SECTION("Izbacivanje izuzetka za nevalidan unos levelId-a pri pozivu "
          "funkcije za cuvanje trenuntnog skora") {
    // Arrange
    Settings* settings = new Settings();
    LevelData* level_data = new LevelData(settings);
    Score* score = new Score(level_data);
